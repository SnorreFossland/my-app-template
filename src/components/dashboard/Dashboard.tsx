...
function Gutter({ onPointerDown }: { onPointerDown: (e: React.PointerEvent) => void }) {
  return (
    <div
      ...
      className="w-6 md:w-8 h-full z-20 flex items-center justify-center px-1"
...
export default function ThreePanels() {
  ...
  return (
    <div ref={containerRef} className="flex gap-2 items-stretch h-[520px]">
      {/* use flex-grow proportions so gutters (fixed px) don't cause overflow */}
  <div style={{ flexBasis: `${sizes[0]}%`, flexGrow: 0, flexShrink: 0 }}>
  ...
  <div style={{ flexBasis: `${sizes[1]}%`, flexGrow: 0, flexShrink: 0 }}>
  ...
  <div style={{ flexBasis: `${sizes[2]}%`, flexGrow: 0, flexShrink: 0 }}>
